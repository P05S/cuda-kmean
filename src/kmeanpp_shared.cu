...
int main() {
    ...
    int data_points_shared_mem_size = BLOCK_SIZE * cols * sizeof(double);
    int centroids_shared_mem_size = K *cols * sizeof(double);
    int centroids_count_shared_mem_size = K * sizeof(int);
    int shared_mem_size = data_points_shared_mem_size + 2*centroids_shared_mem_size + centroids_count_shared_mem_size;
    printf("Shared memory size: %d\n", shared_mem_size);

    for (int iter = 0; iter < MAX_ITER; iter++) {
        cudaMemset(device_new_assignments, 0, sizeof(int));
        assign_clusters<<<blocks, BLOCK_SIZE, shared_mem_size>>>(device_data, device_centroids, device_cluster, device_centroids_change, device_centroids_count, rows, cols, K, device_new_assignments);
        cudaDeviceSynchronize();
    ...
    write_clusters_csv("../output/clusters_kmeanpp_shared.csv", cluster, rows);
    write_centroids_csv("../output/centroids_kmeanpp_shared.csv", centroids, K, cols);
